...
SOL comes from Binance's public monthly kline archives, which need no API key and
publish a SHA256 for every file. One-minute closes are used, so an execution is
priced at the close of the minute it landed in, not at its actual fill price.
"""
...
def fetch(url, cap=256 * 1024 * 1024):
...
class SolPrice:
    ...
    def __init__(self, cache_dir, symbol="SOLUSDT", interval="1m"):
        ...
        self.times, self.closes, self.loaded = [], [], set()

    def _archive(self, month):
        name = f"{self.symbol}-{self.interval}-{month}.zip"
        path = self.cache / name
        url = f"{ARCHIVE}/{self.symbol}/{self.interval}/{name}"
        if path.exists():
            raw = path.read_bytes()
        else:
            raw = fetch(url)
