...
钱包功能已实现地址发现、扣费成本核算、综合排序与报告，并内置两个适配器：`adapters.ledger` 从链上余额增减还原完整账本，`adapters.activity` 产出跟单用的实时成交流。**转入转出、空投、代币互换的成本无法还原，这类钱包会被显式拒绝而不是按零成本估算**，所以真实钱包的拒绝率不低。详见 [从零跑通模拟跟单](docs/RUNBOOK_ZH.md)、[排名口径](docs/WALLET_RANKING.md)、[钱包数据](docs/WALLET_DATA.md) 和 [数据源与费用](docs/DATA_SOURCES.md)。
