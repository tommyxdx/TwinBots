# TwinCryptoBots 1.2.0

两个 bot 和一个独立数据获取器：**Solana 钱包历史表现扫描 + 跟单/模拟交易**。不使用大语言模型、不需要 OpenAI Key，不签署交易、不广播、不真实下单。扫描器按已实现盈亏、成交数、胜率和资源配置给钱包排名；跟单 bot（`follow.enabled`，默认关闭）把入选钱包的新成交镜像到独立模拟账户，见 [docs/COPY_TRADING.md](docs/COPY_TRADING.md)。CEX 独立使用虚拟 $100。默认已替换百倍币扫描。

钱包功能已实现地址发现、扣费成本核算、综合排序与报告，并内置两个适配器：`adapters.ledger` 从链上余额增减还原完整账本，`adapters.activity` 产出跟单用的实时成交流。**转入转出、空投、代币互换的成本无法还原，这类钱包会被显式拒绝而不是按零成本估算**，所以真实钱包的拒绝率不低。候选地址可由 Dune/Birdeye/Solscan 等平台粗筛后统一合并（[SHORTLIST_ZH.md](docs/SHORTLIST_ZH.md)），再由本程序精确排名。详见 [从零跑通模拟跟单](docs/RUNBOOK_ZH.md)、[排名口径](docs/WALLET_RANKING.md)、[钱包数据](docs/WALLET_DATA.md) 和 [数据源与费用](docs/DATA_SOURCES.md)。
...
**不填任何密钥也可以运行：**公开历史下载、GeckoTerminal 地址发现、本地钱包账本排名、Binance 公共订单簿 paper、离线 demo。仅发现地址不会获得完整盈亏；数据缺失时输出 unavailable。是否能连接取决于当地网络及数据源访问政策。行情连接成功也可能没有符合条件的交易；模型无样本或未超过留出集基线时，默认 CEX 会保持现金。

## 需要填写的位置

| 功能 | `.env` | `config.yaml` |
|---|---|---|
| 钱包账本 | 本地文件无需 Key；适配器可用 `WALLET_DATA_API_KEY` | `wallets.source`、`ledger_dir`、`url_template`；见钱包接入文档 |
...
每六小时从近期池成交中发现少量地址，并合并用户名单。每轮最多读取两个钱包的完整账本，核验覆盖、身份、时间和库存后，用移动加权成本计算 7/30/90 天的已实现净盈亏。

排名以 90 天已售成本收益率为主，结合完整平仓数、独立代币数、盈亏比、去掉最大盈利币后的表现及未平仓亏损。胜率展示但不设高胜率门槛；拆单不会增加胜场，本金等比例放大不提高分数。阈值是待验证的研究规则，不是盈利概率。

独立 `scan` 不运行历史下载或模型维护。`python -m examples.wallet_demo` 可零网络生成合成案例报告；真实运行步骤见 [钱包接入](docs/WALLET_DATA.md)。

### 旧代币扫描（仅 `scanner.kind: tokens`）
...
**DEX（仅旧代币模式）：**候选评分 + 新鲜安全检查 + 动量/波动状态筛选；先查买入和反向卖出报价过滤往返成本，再等待设定延迟、重新报价。若输出低于最初最低接受量，模拟整笔回滚，仅扣设定 gas；未上链丢弃不扣 gas；取不到到达报价则记录不可验证、不虚构成交。成功 swap 是整笔原子成交，不伪造部分成交。止损、翻倍后追踪退出、最长持有 24h 都基于反向报价。无法卖出的持仓不会凭空消失，超时无报价按 0 估值并保留库存。
...
`fetch --watch` 每小时运行 CEX 历史下载；仅旧代币模式还运行池发现和历史跟踪。训练单独执行或由 `run` 的每日维护触发。`--pool-pages` 是每个被抽样跟踪池的回溯页数，不会遍历全链；多页仅在该池达到刷新间隔时使用。默认近期池按地址哈希抽样，最多同时跟踪 20 个、跟踪至池龄 35 天，每 12 小时请求历史。抽样独立于评分及后续涨幅，但仍受提供商漏收录、容量和历史可用性影响。
...
```bash
python -m examples.wallet_demo              # 钱包排序反例，全离线，生成 HTML/JSON
python -m twobots demo                       # 原模拟交易合成场景，不是策略收益
python -m unittest discover -s tests -v      # 软件行为测试
python -m twobots scan --once                # 钱包扫描一轮，显示排名及数据缺失
...
核心策略、数据源客户端、纸上账本、模型训练、Telegram 发信、报价适配器、异常处理、报告和启动脚本均提供源码。外部服务的账户/套餐/密钥、钱包完整账本与供应商转换器、完整失败币历史库、付费持仓/资金簇索引器需另行提供；不是包内已经存在的服务。
