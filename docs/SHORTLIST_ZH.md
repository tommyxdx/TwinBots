...
平台能看到永续、借贷、LP，是因为它们**逐个协议解码指令**——为 Drift、Jupiter Perps、Kamino 各写一套解析器。看得进协议内部，但只覆盖它们写过解析器的协议。

本程序用**代币余额增减**，取舍正好相反：任何会移动代币的协议都自动覆盖，包括明天才出现的；但不移动代币的活动（永续、借贷、质押）完全看不见。

两种都不完整，而且**平台的口径无法从外部核对**。实测的例子：Birdeye 盈亏榜上一个地址，它自己的链上现货记录是 50 天 3799 笔、平均每笔 $0.23、总投入 $830、净亏 $21——一个刷量机器人。它凭什么上榜我无法确定。

所以报告里每个钱包都标注了**「谁推荐的」**，和本程序自己算出的数字并排。两边不一致时，以可复核的那一列为准。这个分歧本身就是这套东西存在的理由：一键跟单服务会直接把榜单给你，但不会给你核对它的办法。
