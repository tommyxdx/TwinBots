...
**1.2.0：**默认扫描对象已改为钱包。新的完整账本格式、只读适配器、地址发现和用量见 [WALLET_DATA.md](WALLET_DATA.md)。下文涉及代币评分、池 OHLCV 与百倍训练集的内容仅用于 `scanner.kind: tokens`；其用量估算不适用于钱包模式。
...
默认 `.sqlite3` 只保存聚合 K 线、池元数据、扫描、订单、模型指标和账户状态。价格训练在本机 CPU 上每日最多尝试一次，约万行的线性模型无需 GPU；实际耗时依机器而定。扫描/错误记录保留 35 天；净值、订单和聚合 K 线持续保留。缓存和 SQLite 长期仍会增长，不是零本地存储。需要更长期净值审计，请定期导出报告与备份。
...
可选增强字段：`log_liquidity` 为 ln(1+美元流动性)，`age_hours`，`turnover_h1` 为小时美元量/流动性，`buy_sell_ratio_h1` 为 buys/max(sells,1)，`top10_ex_lp_fraction`，`largest_funding_cluster_fraction`，`creator_bad_rate`。某增强列在完成时间切分和 purge 的训练段中有至少 80% 覆盖才纳入该模型；预测时所需特征至少 75% 已知，否则不输出。其余按训练段中位数填充。
...
默认不调用链上 RPC，因此程序本身不消耗你的 RPC CU；外部索引商可能按 credits/CU/请求计费。`http.max_requests_per_day` 统计应用 HTTP 次数（含重试），不能替代各供应商按权重计费。WebSocket 连接/消息未算 HTTP 次数。默认扫描最多约 432 次/天，若每候选额外查一次安全接口再约 288 次/天；DEX 两仓每 5 分钟反查可额外约 576 次/天，再加历史与 CEX。实际扫描耗时会降低频率，入场/失败重试也会增加请求。达到预算后停止新增 HTTP 请求，已有 WebSocket 可能仍在线；报告会提示数据缺失，报价驱动退出也可能暂停。
