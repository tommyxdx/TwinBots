...
**实验一：排名本身有没有预测力。** 不涉及执行，最干净。

```bash
python -m twobots forward
```

每天自动存一份排名快照。`forward` 取相隔一周的两份，用后一份的 **7 天窗口收益率**（恰好覆盖排名之后那一周）对比前一份排名的上半区与下半区：

```json
{"ranked_at": ..., "measured_at": ..., "wallets": 12,
 "top_half_mean_roi7": 0.18, "bottom_half_mean_roi7": -0.04, "separation": 0.22}
```

`separation` 持续为正才是证据，**单个数据点不算**。这个实验没有延迟、滑点和手续费的噪声，直接回答"排名选出来的钱包是不是真的继续表现好"。

快照**无法事后补**——要做这个实验，现在就得让它跑着。

**实验二：跟单在扣掉手续费后还剩多少。** 模拟账户按 `platform_fee_fraction`（默认 1%/腿）收费，和平台一致。

两腿共 2% 是**每笔交易的硬地板**：领投的平均往返如果净赚不到 2%，通过平台跟单在数学上就不可能盈利，跟得再准也没用。

注意一个结构性风险：**我们只能复制看得见的现货成交**。如果领投的 edge 在永续（见 `activity_partly_invisible` 标记），我们复制的可能只是它的对冲腿或噪声，纸面跟单大概率亏损——这本身就是对那个假设的检验。
...
`wallets.min_closed_cycles` 决定一个钱包要有多少次完整平仓才够格排名。默认 10，实测真实钱包很难达到——64 个候选筛下来，能还原的几个只有 3-4 个周期。

**如果为了让管线跑起来而下调（例如 5），那么这次观察期证明的是"阈值 5 的钱包有没有 edge"，不是阈值 10 的。** 之后调回 10 是收紧，方向偏保守，但**之前的结果不能直接当作新配置的证据**——严格来说要重跑一个观察期。

无论定在几，**一旦观察期开始就不能再动**。中途改等于在结果上拟合参数。
...
进程被系统 `Killed`、日志戛然而止、`dmesg` 里有 `Out of memory: Killed process ... (python)`，说明内核把进程杀了。这不是程序报错，是机器内存不够，**当前周期里所有钱包的进度全部丢失，而且不留下任何原因记录**。

峰值出现在还原一个长历史钱包的时候，现在最坏情况（6 万笔交易 + 400 天价格序列）约 **122 MB**：

| 组成 | 占用 |
|---|---|
| Python 本身 | 15 MB |
| twobots 全部模块 | +10 MB |
| 400 天 SOL 价格序列 | +21 MB |
| 6 万条 normalized 交易 | +73 MB |
| 转成账本行（边转边释放） | +3 MB |

几件已经做掉的事：

- **numpy / pandas / scikit-learn 改成延迟导入。** 这三个库常驻 110 MB，但只有 CEX bot 和代币扫描用得到。`scanner.kind: wallets` 时现在完全不加载。
- **90 天窗口之外用小时线。** 三年分钟线是 160 万根，加载时旧数组、合并字典、新数组同时在内存里，峰值是稳态的三倍。窗口外的成本基准用小时收盘价定价，误差远小于 1%，`fine_days` 可调。
- **`ledger_memory_ceiling_mb`：常驻内存闸门。** 默认 `0` = 按机器总内存的 70% 自动算（913 MB 的机器 → 639 MB）。还原过程中每 2048 笔检查一次，超了就**放弃这一个钱包**，在报告里记成 `memory_ceiling`，其余钱包照常继续。宁可丢一个钱包，也不要被内核杀掉整个周期。

**还是建议加 swap。** 上面这些把峰值压下来了，但零 swap 意味着任何一次瞬时超额都是立即死亡，没有缓冲：

```bash
sudo fallocate -l 2G /swapfile && sudo chmod 600 /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile && echo '/swapfile none swap sw 0 0' | sudo tee -a /etc/fstab
```

swap 不会让程序变快——被换出去的页读回来很慢——但它把"立即被杀"变成"慢一下"。在这种规格的机器上这笔买卖是划算的。
