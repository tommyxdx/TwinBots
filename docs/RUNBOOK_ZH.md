...
第一批真实跟单里，两笔亏损都是同一个原因：**领投自己的单子就是行情**。

| | 领投下单 | 我们付的价 vs 领投 | 领投结果 | 我们结果 |
|---|---|---|---|---|
| 亏损 1 | 一次 $34,238 | 4.67 倍 | +164% | −62.6% |
| 亏损 2 | 一次 $21,047 | 3.24 倍 | +77% | −77.6% |
| 盈利 | $1,130 × 7 次分批 | 1.06 倍 | 继续加仓 | +291% |

在很薄的池子里，一笔大单能在几十秒内把价格推高好几倍。我们看到信号时，行情已经被领投做完了——买在他推出来的高点，他卖的时候又砸在我们前面。**排名能找出赚钱的钱包，但赚钱的方式不一定能跟。**

### 追价（chase）怎么算

追价 = 我们每美元能买到的币比领投少多少。100% 意味着我们付了领投的两倍。用原始单位（不除以 decimals）比较，价格再小的币也能精确比较。

- `max_chase_fraction: 0.25`：追价超过 25% 就不跟。设 0 关闭。
- 领投成交价未知（活动流没给数量）时，正式账本记为 `CHASE_UNVERIFIABLE` 并拒绝——无法确认行情还在前面。

### 领投级筛选：`impact_trader`

单笔拦截只能防止买到一次拉盘。如果一个领投最近 `min_chase_samples` 个以上信号的**追价中位数**超过门槛，说明他的利润主要来自自己砸出来的价格冲击，直接从跟单名单剔除。这就是「找策略稳定、不靠大资金暴力拉盘的钱包」的具体实现。

观察窗口 `chase_window_days` 过后记录会过期，被剔除的领投会重新接受检验——钱包的行为会变。

### 影子账本

跟正式账本看**同一批信号**，但不受风控暂停、仓位上限、单领投预算、追价闸约束，每个信号都跟。它不交易任何真东西，只回答两个问题：

1. **跟领投本身能不能赚钱**——正式账本被风控锁死时，影子账本照常记录。第一次锁死曾让数据收集中断了 12 小时。
2. **追价闸值不值**——被闸门拦下的交易，只有影子账本真的做了。报告里「追价闸：拦下的那些到底怎么样」按追价把影子账本的交易分成放行组和拦下组。如果拦下组并不比放行组差，说明闸门在白白放弃交易，门槛该调。

影子账本的 HTTP 请求用**独立预算**（`shadow_max_requests_per_day`），永远不会挤占正式账本的预算。否则预算耗尽 → 报价失败 → 持仓按 0 估值 → 正式账本因回撤被锁死，影子账本反而把正式账本弄停。

### 重置纸面账户

先停掉 bot（进 screen 按 `Ctrl+C`），然后：

```bash
cd ~/Twinbots && ~/.venv/bin/python -m twobots reset
```

默认同时重置 `copy` 和 `shadow`（它们是同一个实验）。只重置某一个用 `--venue copy` 或 `--venue shadow`，可以重复写。

**不会删除任何东西**：旧的订单和估值记录被改名为 `copy@<时间>`，旧账户状态存为 `archive:account:copy@<时间>`，以后仍可查。报告和风控只看新账户。bot 还在跑时命令会拒绝执行——正在运行的账本会把旧状态直接写回去。
