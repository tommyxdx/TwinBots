...
| 情况 | 处理 | 理由 |
|---|---|---|
| USDC/SOL 充值提现 | **跳过** | 现金进出不进入代币成本会计，拒绝它会让几乎所有钱包出局 |
| 创建 ATA 扣的 SOL 租金 | **计入成本** | 每条计价腿各自折美元再相加，不因为"两种计价资产"而拒绝 |
| 失败交易 | 记为独立 gas 费用 | 抢跑失败也是策略成本 |
| 代币转入 / 空投 | **拒绝整个钱包** | 成本未知，按零成本计会凭空造出利润 |
| 代币换代币（BONK→WIF） | **拒绝整个钱包** | 两条腿都是研究资产，没有可信美元价 |
| 历史不足 90 天 | 拒绝 | 排名硬性要求 |

实际后果：**长期活跃的真实钱包被拒的比例会很高**，因为转入转出太常见。这是设计取舍不是 bug。如果拒绝率高到没有候选，说明需要在适配器上层做成本对账（把转入按转出方的成本继承过来），那是一个独立的工程，本版没做。
